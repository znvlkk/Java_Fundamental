...
public class FileDemo {
	...
	public void changeFile(String path) {
		// path => c:\dev\test\VS.exe = > c:\dev\test\현제시간. exe
		File f1 = new File(path);
	...
	public static void main(String[] args) {
		/*
		 * FileDemo f1 = new FileDemo(); f1.mkdirs("c:\\dev\\test\\2019\\08\\01");
		 * 
		 * File f2 = new File("c:\\dev\\test\\VSCodeUserSetup-x64-1.36.1.exe"); long
		 * fileSize = f2.length() / 1024; System.out.println(f1.getComma(fileSize) +
		 * "KB"); boolean isFile = f2.isFile(); if (isFile) { String path =
		 * f2.getPath(); // f2의 전체경로를 반환한다 if (path.endsWith("pdf")) {
		 * System.out.println("PDF 파일"); } else if (path.endsWith("exe")) {
		 * System.out.println("응용프로그램"); } else if (path.endsWith("txt")) {
		 * System.out.println("텍스트문서"); } System.out.println(path);
		 * 
		 * } else { System.out.println("파일 폴터"); }
		 * 
		 * File f3 = new File("c:\\dev\\test", "VS.exe"); f2.renameTo(f3); // f2파일을
		 * f3이름으로 변경
		 * 
		 * f1.changeFile("c:\\dev\\test\\VS.exe");
		 */
		File f4 = new File("c:\\dev\\test", "VS.exe");
